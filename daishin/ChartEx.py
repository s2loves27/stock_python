...
class CpStockChart:
    ...
    def RequestMT(self, code, dwm, count, caller):
        ...
        self.objStockChart.SetInputValue(5, [0, 1, 2, 3, 4, 5, 6, 8])  # 요청항목 - 날짜, 시간,시가,고가,저가,종가,거래량
        ...
        for i in range(len):
            ...
            caller.per.append(self.objStockChart.GetDataValue(6, i))
            caller.vols.append(self.objStockChart.GetDataValue(7, i))
...
class MyWindow(QMainWindow):
    ...
    def btchart7_clicked(self):
        ...
        if (len(self.times) == 0):
            ...
            df = pd.DataFrame(chartData, columns=['일자', '시간', '시가', '고가', '저가', '종가','전일대비', '거래량'])
