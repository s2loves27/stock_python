...
class Cp6033:
    ...
    def requestJango(self, caller):
        while True:
            ...
            balance = {}
            balance['계좌명'] = self.acc_name
            balance['결제잔고수량'] = self.objRq.GetHeaderValue(1)
            balance['체결잔고수량'] = self.objRq.GetHeaderValue(2)
            balance['평가금액'] = self.objRq.GetHeaderValue(3)
            balance['평가손익'] = self.objRq.GetHeaderValue(4)
            balance['수신개수'] = self.objRq.GetHeaderValue(7)
            balance['수익률'] = self.objRq.GetHeaderValue(8)
            print(balance)

            caller.balance = balance
            for i in range(balance['수신개수']):
                item = {}
                ...
                item['손익단가'] = self.objRq.GetDataValue(18,i) #손익 단가
                item['수익률'] = self.objRq.GetDataValue(11,i) # 수익률
                # 매입금액 = 장부가 * 잔고수량
